...
// Interactive Icon Component - Versão Simplificada para Debug
const InteractiveIcon: React.FC<{
  ...
  mouseX: number;
  mouseY: number;
}> = ({ Icon, position, color, delay, mouseX, mouseY }) => {
  console.log(`🎨 InteractiveIcon rendering: ${color}`);
  
  return (
    <div
      className={`absolute ${color} pointer-events-none z-10 border-4 border-red-500 p-2`}
      style={position}
    >
      <Icon className="h-10 w-10" />
      <div className="text-xs text-white bg-black p-1 rounded mt-1">
        {color.split('-')[1]} - Mouse: {mouseX},{mouseY}
      </div>
    </div>
  );
};
    ...
    >
      ...
      <div className="relative w-full max-w-6xl mx-auto text-center z-10 flex flex-col justify-center h-full py-8">
        ...
        >
          ...
          {/* Enhanced Main Title - Responsivo */}
          <motion.h1 
          ...
          {/* Enhanced Description - Responsivo */}
          <motion.div
        ...
        {/* Enhanced CTA Section - Responsivo */}
        <motion.div
        ...
        {/* Enhanced Scroll Indicator - Posicionado no final */}
        <motion.div
